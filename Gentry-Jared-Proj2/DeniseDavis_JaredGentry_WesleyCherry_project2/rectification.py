...
def rectification(T,Wr,Wl,R,imgL,imgR):
    ...
    Rlrect = -1 * Rlrect  # this seems to prevent 180 degree rotation of image
    # print(v1,v1.shape)
    # print(v2,v2.shape)
    # print(v3,v3.shape)
#    print(Rlrect, Rlrect.shape)
    ...
    newImgL=numpy.zeros((row,col,3), numpy.uint8) # adding padding here if needed
...
#    cv2.imwrite('oldleft.png',imgL)
    cv2.imwrite('rectified_left.png',newImgL)
    # for some reason, need to flip image
    flip_img = cv2.flip( newImgL, 1 )
    cv2.imwrite('rectified_left_flipped.png',flip_img)
    # cv2.waitKey(0)
...
#    cv2.imwrite('oldright.png',imgR)
    cv2.imwrite('rectified_right.png',newImgR)
    # for some reason, need to flip image
    flip_img = cv2.flip( newImgR, 1 )
    cv2.imwrite('rectified_right_flipped.png',flip_img)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
